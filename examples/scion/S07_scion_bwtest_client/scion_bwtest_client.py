...
def main(path_to_logs):
    ...
    # AS-153 bwtestclient
    node = as153.createHost('bwtestclient').joinNetwork('net0', address='10.153.0.31')
    node.addSharedFolder("/var/log", os.path.abspath(path_to_logs)) # make logs of bwtestclient available on host
...
if __name__ == "__main__":
    ...
    parser.add_argument("--path_to_logs", "-p", help="Path to the directory where the logs of the bwtestclient will be stored")

    args = parser.parse_args()

    if not args.path_to_logs:
        print("Please provide the path to the directory where the logs of the bwtestclient will be stored")
        exit(1)
    
    main(args.path_to_logs)